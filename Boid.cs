...
namespace Boids
{
    public class Boid
    {
        ...
        private void Flock(List<Boid> boids)
        {
            ...
            {
                var distance = Distance(Position, boid.Position);
        ...
        {
            var val = Math.Pow(p1.X - p2.X, 2) + Math.Pow(p1.Y - p2.Y, 2);
        ...
        private void CheckSpeed()
        {
            ...
            var val = Distance(new PointF(0f, 0f), new PointF(dX, dY));
            if (val > s)
            {
                dX = dX * s / val;
                dY = dY * s / val;
